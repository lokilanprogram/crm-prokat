<x-app-layout>
    ...
    <main class="w-full px-2 sm:px-4 py-6 min-h-screen">
        ...
        <div class="bg-gray-200 rounded-xl p-4 mb-6 flex items-center justify-between">
            ...
            <div class="flex items-end gap-14 pr-20">
                ...
                <div class="flex flex-col items-center">
                    <i class="bi bi-credit-card text-3xl text-gray-800 mb-2"></i>
                    <span class="text-lg font-semibold">
                    <span class="text-lg font-semibold">Касса: <span class="text-xl font-bold">33&nbsp;866.4&nbsp;<span class="text-gray-600">₽</span></span></span>                    </span>
                </div>
            </div>
        </div>
        ...
        <!-- СКИДКИ: заголовок и кнопка -->
        <div class="flex items-center gap-2 mb-2 mt-4">
            <span class="font-bold text-base">Скидки. Справочник</span>
            <button onclick="openDiscountModal()" class="bg-red-600 hover:bg-red-700 text-white font-semibold px-3 py-1.5 rounded text-sm flex items-center gap-1">
                + НОВАЯ СКИДКА
            </button>
        </div>
        ...
        <div class="overflow-auto mb-6">
            <table class="w-[600px] text-sm border border-[#c5d1e4] bg-white" style="border-collapse:collapse;">
                <thead class="bg-blue-50 text-xs uppercase">
                    <tr>
                        <th class="p-2 border border-[#c5d1e4] text-center">Активность</th>
                        <th class="p-2 border border-[#c5d1e4] text-center">Название</th>
                        <th class="p-2 border border-[#c5d1e4] text-center">Значение</th>
                        <th class="p-2 border border-[#c5d1e4] text-center">Описание</th>
                        <th class="p-2 border border-[#c5d1e4] text-center"></th>
                        <th class="p-2 border border-[#c5d1e4] text-center"></th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td class="p-2 border border-[#c5d1e4] text-center"><span class="text-green-700">&#10003;</span></td>
                        <td class="p-2 border border-[#c5d1e4] text-center">0%</td>
                        <td class="p-2 border border-[#c5d1e4] text-center">0</td>
                        <td class="p-2 border border-[#c5d1e4] text-center">ничего не давать...</td>
                        <td class="p-2 border border-[#c5d1e4] text-center"><button class="px-2 py-1 bg-gray-200 rounded text-xs" disabled>Удалить</button></td>
                        <td class="p-2 border border-[#c5d1e4] text-center"><button class="px-2 py-1 bg-[#337AB7] text-white rounded text-xs">Изменить</button></td>
                    </tr>
                    <tr>
                        <td class="p-2 border border-[#c5d1e4] text-center"><span class="text-green-700">&#10003;</span></td>
                        <td class="p-2 border border-[#c5d1e4] text-center">5%</td>
                        <td class="p-2 border border-[#c5d1e4] text-center">5</td>
                        <td class="p-2 border border-[#c5d1e4] text-center"></td>
                        <td class="p-2 border border-[#c5d1e4] text-center"><button class="px-2 py-1 bg-gray-200 rounded text-xs" disabled>Удалить</button></td>
                        <td class="p-2 border border-[#c5d1e4] text-center"><button class="px-2 py-1 bg-[#337AB7] text-white rounded text-xs">Изменить</button></td>
                    </tr>
                    <!-- ...дальше другие строки -->
                </tbody>
            </table>
        </div>
        ...
        <div id="discount-modal" class="fixed z-50 inset-0 bg-black bg-opacity-30 flex items-center justify-center hidden">
            <div class="bg-white rounded-xl shadow-2xl min-w-[400px] max-w-[430px] border border-gray-300 relative">
                    <div class="bg-sky-700 text-white px-4 py-2 rounded-t-xl flex justify-between items-center">
                        <span class="font-semibold text-base">Создание новой скидки</span>
                        <button onclick="closeDiscountModal()" class="text-white text-2xl font-bold leading-none">&times;</button>
                    </div>
                <form class="p-6">
                    <div class="flex items-center gap-3 mb-3">
                        <label class="block text-sm w-[140px]">Название скидки</label>
                        <input type="text" class="border rounded flex-1 px-2 py-1 text-sm" placeholder="">
                    </div>
                    <div class="flex items-center gap-3 mb-3">
                        <label class="block text-sm w-[140px]">Значение скидки</label>
                        <input type="number" class="border rounded flex-1 px-2 py-1 text-sm" value="0">
                    </div>
                    <div class="flex items-center gap-3 mb-3">
                        <label class="block text-sm w-[140px]">Описание скидки</label>
                        <input type="text" class="border rounded flex-1 px-2 py-1 text-sm" placeholder="">
                    </div>
                    <div class="flex items-center gap-3 mb-3">
    ...
    <script>
        // Dropdowns (Журналы, Справочники)
        function toggleDropdown(id) {
            document.querySelectorAll('.relative .absolute').forEach(e => {
                if (e.id !== id) e.classList.add('hidden');
            });
            document.getElementById(id).classList.toggle('hidden');
        }
        document.addEventListener('click', function(e) {
            ['journals-btn', 'refs-btn'].forEach(btnId => {
                const btn = document.getElementById(btnId);
                const dropdown = document.getElementById(btnId.replace('-btn', '-dropdown'));
