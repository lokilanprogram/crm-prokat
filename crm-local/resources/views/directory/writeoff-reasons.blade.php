...
<!-- resources/views/directory/writeoff-reasons.blade.php -->

<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Справочник. Причины списания</h2>
    </x-slot>
    ...
    <main class="w-full px-2 sm:px-4 py-6">
        ...
        <div class="flex items-center mb-3 mt-6">
            <span class="text-base font-semibold text-gray-700 mr-3">
                <i class="bi bi-x-circle text-lg text-red-500"></i>
                Причины списания оборудования
            </span>
        </div>

        <button onclick="document.getElementById('create-reason-modal').classList.remove('hidden')"
            class="bg-red-600 hover:bg-red-700 text-white font-semibold px-4 py-2 rounded text-xs mb-3">
            + НОВАЯ ПРИЧИНА
        </button>

        <!-- Таблица -->
        <div class="overflow-auto rounded border bg-white w-[270px]">
            <table class="w-full text-sm border-collapse">
                <thead>
                    <tr class="bg-blue-50 text-xs uppercase">
                        <th class="border px-2 py-1 text-left">№ п.п.</th>
                        <th class="border px-2 py-1 text-left">Наименование</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td class="border px-2 py-1 text-center">1</td>
                        <td class="border px-2 py-1">Украдено</td>
                    </tr>
                    <tr>
                        <td class="border px-2 py-1 text-center">2</td>
                        <td class="border px-2 py-1">Сломано</td>
                    </tr>
                    <tr>
                        <td class="border px-2 py-1 text-center">3</td>
                        <td class="border px-2 py-1">Потеряно</td>
                    </tr>
                    <tr>
                        <td class="border px-2 py-1 text-center">4</td>
                        <td class="border px-2 py-1">Списан на запчасти</td>
                    </tr>
                </tbody>
            </table>
        </div>
    ...
    <div id="create-reason-modal" class="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center hidden z-50">
        <div class="bg-white rounded-xl shadow-xl border border-gray-300 relative" style="width: 420px; max-width: 100%;">
            <div class="bg-[#337AB7] text-white text-sm font-semibold px-4 py-2 rounded-t-xl flex items-center justify-between mb-4">
                <span>Новая причина списания</span>
                <button type="button"
                        onclick="document.getElementById('create-reason-modal').classList.add('hidden')"
                        class="text-white text-xl font-bold leading-none hover:text-gray-300 transition">&times;
                </button>
            </div>
            <form class="p-4">
                <div class="flex items-center gap-3 mb-3">
                    <label class="block text-sm w-[110px]">Наименование</label>
                    <input type="text" placeholder="" class="border rounded flex-1 px-2 py-1 text-sm" />
                </div>
                <div class="flex justify-end pt-3">
                    <button type="submit" class="bg-[#337AB7] text-white px-4 py-2 rounded text-sm font-semibold mr-2" disabled>
                        <i class="bi bi-save"></i> СОХРАНИТЬ ЭЛЕМЕНТ
                    </button>
                    <button type="button" onclick="document.getElementById('create-reason-modal').classList.add('hidden')" class="bg-red-500 text-white px-4 py-2 rounded text-sm font-semibold">
                        ОТМЕНА
                    </button>
                </div>
            </form>
